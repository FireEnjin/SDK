...
class AuthService {
    ...
    async initializePushNotifications(onMessageCallback, options) {
        try {
            const messaging$1 = messaging.getMessaging(this.app);
            if (onMessageCallback && typeof onMessageCallback === "function") {
                messaging.onMessage(messaging$1, onMessageCallback);
            }
            const vapidKey = options?.vapidKey;
            let messagingToken = await messaging.getToken(messaging$1, {
                vapidKey,
            ...
            if (!messagingToken) {
                ...
                if (permission === "granted") {
                    ...
                    messagingToken = await messaging.getToken(messaging$1, {
                        vapidKey,
                        serviceWorkerRegistration: await navigator.serviceWorker.getRegistration(),
                    });
                }
...
class DatabaseService {
    ...
    constructor(options) {
        ...
        if (!this.app) {
            try {
                this.app = app$1.initializeApp(options?.config);
                console.log("Initializing Firebase App...", this.app);
            }
            catch (e) {
                console.log(e);
